...
### 8.2 ローカル専用状態管理 (local-index.json) の導入

現在の `sync-index.json` はクラウド上で全デバイスに共有されるため、「あるデバイスにとってのBaseリビジョン」が他デバイスの更新で上書きされてしまう構造上の制約がある。

**計画**: 各デバイスが「自分専用の同期状態」を管理する仕組みへ移行する。
...
- `local-index.json` のハッシュを確定的なBaseとして使用でき、`findCommonAncestorHash` のような推測ロジックへの依存が解消される
- オフライン作業後の同期やRevert後の再編集など、複雑なシナリオでの堅牢性が向上する
