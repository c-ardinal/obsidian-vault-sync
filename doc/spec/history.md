...
### 6.2 差分表示 (Diff Viewer)

高度な差分エンジンによる、直感的かつ詳細な比較機能を提供する。

- **表示モード**: ユニファイド（1ペイン、行内差分あり）およびスプリット（左右分割）を切り替え可能
- **同期スクロール**: スプリット表示時、左右のテキストが物理行レベルで完全に同期してスクロール・整列される
- **インライン・ハイライト**: 1行内の一部が変更された場合、該当箇所のみを文字レベルでハイライト表示
- **差分フォーカス (Context View)**:
    - **差分のみ表示**: 変更がない行を非表示にし、重要な変更点に集中できる
    - **コンテキスト設定**: 差分行の周囲に表示する「変更のない行（コンテキスト）」の行数を動的に変更可能
- **差分ジャンプ (Navigation)**:
    - 変更箇所（差分ブロック）間をスムーズに移動するための「前へ」「次へ」ボタンを搭載
    - ジャンプ時はターゲット行を中央に表示し、ブロック全体をアウトラインで強調表示
    - 最後の差分から最初へ、または最初の差分から最後へ「リング（ループ）」でのナビゲーションが可能
- **制限**: 2MBを超えるファイルは本体の制限によりDiff表示不可（ダウンロードのみ）
