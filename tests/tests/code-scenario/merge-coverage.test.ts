...
import { findCommonAncestorHash, perform3WayMerge, pullFileSafely } from "../../../src/sync-manager/merge";
...
describe("merge.ts additional coverage", () => {
    ...
    describe("Safety Guard warning path", () => {
        it("should log warning when remote overwrites our push", async () => {
            ...
            const warningLogged = device.logs.some(l =>
                l.includes("overwritten") || l.includes("Forcing merge")
            );
            expect(warningLogged).toBe(true);
        });
    });
    ...
    // ═══════════════════════════════════════════════════════════════════
    // Write error handling with recovery (lines 1943-1945)
    // ═══════════════════════════════════════════════════════════════════
    ...
    describe("Merge lock acquisition failure", () => {
        it("should handle when merge lock is already held", async () => {
            ...
            const { acquireMergeLock, checkMergeLock } = await import("../../../src/sync-manager/state");
    ...
    // ═══════════════════════════════════════════════════════════════════
    // Remote deletion with conflict (lines 692-702)
    // ═══════════════════════════════════════════════════════════════════
    describe("Remote deletion with conflict", () => {
        it("should handle remote deletion when local is modified", async () => {
            ...
            const result = await pullFileSafely(ctx, {
                path: FILE_PATH,
    ...
    // ═══════════════════════════════════════════════════════════════════
    // perform3WayMerge error handling (lines 1719-1722)
    // ═══════════════════════════════════════════════════════════════════
